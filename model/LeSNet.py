...
class LeSNet(nn.Module):
    def __init__(
        ...
        self.linear_video_mot = nn.Linear(feature_dim, d_model)
        self.norm_video_mot = nn.LayerNorm(d_model, eps=1e-12)
        #####################clip position###################
    ...
    def get_vqa_embedding_rfcpos(self, video, language, language_lens):
        ...
        video_f = self.norm_video(video_f) #(64,8,16,512)
        
        video_o = self.linear_video_mot(video_o) #mot_feat
        video_o = gelu(video_o)
        video_o = self.norm_video_mot(video_o) #(64,8,512)
        
        bsize, numc, numf, fdim = video_f.size()
        ...
        short_mask = get_mask(torch.tensor([numf]*bsize*numc, dtype=torch.long), numf).cuda()
        X = self.ntrans(X.reshape(bsize*numc, numf, -1), short_mask)[0]
        #X = X.reshape(bsize*numc, numf, -1)#.permute(0,2,1,3)
        ...
        hd_dim = X.shape[-1]         # 512
        #X = X.reshape(bsize*numc*numf, numr, hd_dim)   # [512, 4, 5, 512]-> [2048, 5, 512]

        X_o = X  

        satt = self.satt_pool(X_o)
        X_o = torch.sum(X_o*satt, dim=-2)
        X_o = X_o.view(bsize, numc, -1)
        
        video = self.merge_fr(torch.cat([X_o,video_o], dim=-1))
    ...
    def forward(
        ...
        if mode == "vqa":
            ...
            if self.baseline == "qa":
            ...
            elif len(seq_len.squeeze().shape) == 1:   # x 
                ...
                video_proj = self.get_vqa_embedding_rfcpos(video, answer_w, seq_len)  # 64,8,512
